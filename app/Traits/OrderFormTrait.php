...
trait OrderFormTrait
{
    ...
    public function recalculateTotal()
    {
        ...
        $subtotal += $this->calculateAdditionalTotal();
        
        // Adicionais dos dependentes
        $subtotal += $this->calculateDependentsTotal();
        
        // Aplicar desconto
        $this->total = $this->applyDiscount($subtotal);
    }

    private function applyDiscount($subtotal)
    {
        if (empty($this->discount_type) || empty($this->discount_value) || $this->discount_value <= 0) {
            return $subtotal;
        }
        
        if ($this->discount_type === 'R$') {
            // Desconto em valor fixo
            $total = $subtotal - $this->discount_value;
            // Garante que não fique negativo
            return max(0, $total);
        } elseif ($this->discount_type === '%') {
            // Desconto percentual
            $percentual = min(100, max(0, $this->discount_value)); // Limita entre 0 e 100
    ...
    {
        // Se trocar o tipo de desconto, recalcula
        $this->recalculateTotal();
    }

    public function updatedDiscountValue()
    {
        // Se alterar o valor do desconto, recalcula
        $this->recalculateTotal();
    }
